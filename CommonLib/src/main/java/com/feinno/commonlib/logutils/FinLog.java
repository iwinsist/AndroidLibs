...
/**
 * 飞聊的Log工具类，对系统的Log进行了封装，并加入了system.out和文件的输出方式。
 */
public class FinLog
{
	...
	/** 日志级别 */
//	public static int stcLevel = ERROR;
	...
	public static void i(String content)
	{
		if (stcLevel <= INFO)
		{
			Log.i("finger", thread_toggle ? ("[" + getCurrentThreadName() + "]" + content) : content);
		}
	}
	...
	public static void d(String content)
	{
		if (stcLevel <= DEBUG)
		{
			Log.d("finger", thread_toggle ? ("[" + getCurrentThreadName() + "]" + content) : content);
		}
	}
	...
	public static void e(String content)
	{
		if (stcLevel <= ERROR)
		{
			Log.e("finger", thread_toggle ? ("[" + getCurrentThreadName() + "]" + content) : content);
		}
	}
